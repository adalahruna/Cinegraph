...
export class OrderService {
  ...
  static async createOrder(cartItems, orderData = {}) {
    try {
      ...
      console.log('Creating order for user:', { userId, userEmail, hasProfile: !!profile })
      ...
      // Stock updates are handled asynchronously (fire-and-forget for speed)
      ...
      setTimeout(async () => {
        for (const item of cartItems) {
          try {
            ...
            console.error(`Stock update error for ${item.id}:`, err)
          }
        }
      }, 100) // Run after 100ms to not block response
